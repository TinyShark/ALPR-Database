...
export async function removeKnownPlate(plateNumber) {
  ...
  try {
    ...
    // First, delete all misreads associated with the parent plate
    await client.query(
      "DELETE FROM known_plates WHERE parent_plate_number = $1",
      [plateNumber]
    );

    // Then, delete the parent plate
    await client.query(
...
export async function getAllPlatesWithKnownInfo({ 
  ...
  console.log('DB: Starting query with sort:', { sortField, sortOrder });

  // Build the WHERE clause for filtering
  let whereClause = 'kp.parent_plate_number IS NULL';
  ...
  const countResult = await pool.query(
    `SELECT COUNT(DISTINCT p.plate_number) as total
     FROM plates p
     LEFT JOIN known_plates kp ON p.plate_number = kp.plate_number
     WHERE ${whereClause}`,
    queryParams
  ...
  const totalFilteredPlates = parseInt(countResult.rows[0].total);

  // Main query with correct sorting of parent plates by total occurrences
  const result = await pool.query(
    `WITH read_counts AS (
      SELECT 
        plate_number,
    ...
    parent_reads AS (
      SELECT 
        p.plate_number,
        COALESCE(rc.occurrence_count, 0) + 
        COALESCE((
          SELECT SUM(rc_child.occurrence_count)
          FROM known_plates kp_child
          LEFT JOIN read_counts rc_child ON kp_child.plate_number = rc_child.plate_number
          WHERE kp_child.parent_plate_number = p.plate_number
        ), 0) as total_occurrences,
        GREATEST(
          COALESCE(rc.last_seen_at, '1970-01-01'::timestamp),
          COALESCE((
            SELECT MAX(rc_child.last_seen_at)
            FROM known_plates kp_child
            LEFT JOIN read_counts rc_child ON kp_child.plate_number = rc_child.plate_number
            WHERE kp_child.parent_plate_number = p.plate_number
    ...
    FROM plates p
    LEFT JOIN known_plates kp ON p.plate_number = kp.plate_number
    LEFT JOIN parent_reads ON p.plate_number = parent_reads.plate_number
    LEFT JOIN plate_tags pt ON p.plate_number = pt.plate_number
    LEFT JOIN tags t ON pt.tag_id = t.id
    WHERE ${whereClause}
    GROUP BY 
  ...
  console.log('DB: Returning sorted data:', {
    sortField,
    sortOrder,
    sampleData: result.rows.slice(0, 3).map(row => ({
      ...
      misreads_count: row.misreads?.length || 0
    }))
  });
