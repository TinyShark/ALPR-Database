...
export async function getPlateReads({
  ...
  if (filters.tag && filters.tag !== "all") {
    ...
    conditions.push(`
      EXISTS (
        WITH matching_plates AS (
          -- Get plates that directly have the tag
          SELECT plate_number 
          FROM plate_tags pt 
          ...
          -- Get misreads whose parents have the tag
          SELECT kp.plate_number
          FROM known_plates kp
          JOIN plate_tags pt ON pt.plate_number = kp.parent_plate_number
          JOIN tags t ON pt.tag_id = t.id 
          WHERE t.name = $${paramIndex}
        )
        SELECT 1 
        FROM matching_plates
        WHERE matching_plates.plate_number = pr.plate_number
      )
    `);
    
    // Add debug logging query - modified to only show relevant tag matches
    const debugQuery = `
      WITH matching_plates AS (
        -- Direct matches
        SELECT plate_number, 'direct' as match_type, t.name as tag_name
        FROM plate_tags pt 
        JOIN tags t ON pt.tag_id = t.id 
        WHERE t.name = $1
        ...
        -- Inherited matches
        SELECT kp.plate_number, 'inherited' as match_type, t.name as tag_name
        FROM known_plates kp
        JOIN plate_tags pt ON pt.plate_number = kp.parent_plate_number
        JOIN tags t ON pt.tag_id = t.id 
        WHERE t.name = $1
      )
      SELECT * FROM matching_plates
  ...
  if (filters.cameraName && filters.cameraName !== "all") {
  ...
  if (filters.dateRange?.from && filters.dateRange?.to) {
    // First add the date range condition
    ...
    // Then handle time range if specified
    if (filters.timeFrom || filters.timeTo) {
      const now = new Date();
      const currentTimeInMinutes = now.getHours() * 60 + now.getMinutes();
      const selectedDate = new Date(filters.dateRange.from);
      ...
      if (filters.timeFrom && filters.timeTo) {
        ...
        if (filters.dateRange.from === filters.dateRange.to) {
          ...
          if (toTimeInMinutes < fromTimeInMinutes || (isToday && fromTimeInMinutes > currentTimeInMinutes)) {
            // Handle overnight range or future time by including previous day
            const prevDay = new Date(filters.dateRange.from);
            prevDay.setDate(prevDay.getDate() - 1);
            const prevDayStr = prevDay.toISOString().split('T')[0];

            // Update date range to include previous day
            conditions[conditions.length - 1] = `pr.timestamp::date BETWEEN $${paramIndex - 2} AND $${paramIndex - 1}`;
            queryValues[queryValues.length - 2] = prevDayStr;
            queryValues[queryValues.length - 1] = filters.dateRange.to;
            countValues[countValues.length - 2] = prevDayStr;
            countValues[countValues.length - 1] = filters.dateRange.to;

            conditions.push(`
              (
                (pr.timestamp::date = $${paramIndex - 2} AND
                 EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) >= ${fromTimeInMinutes})
                OR
                (pr.timestamp::date = $${paramIndex - 1} AND
                 EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) <= ${toTimeInMinutes})
              )
            `);
          } else {
            conditions.push(`
              EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) 
              BETWEEN ${fromTimeInMinutes} AND ${toTimeInMinutes}
            `);
          }
        } else {
          // Multi-day selection
          conditions.push(`
            (
              (pr.timestamp::date = $${paramIndex} AND
               EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) >= ${fromTimeInMinutes})
              OR
              (pr.timestamp::date = $${paramIndex + 1} AND
               EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) <= ${toTimeInMinutes})
              OR
              (pr.timestamp::date > $${paramIndex} AND pr.timestamp::date < $${paramIndex + 1})
            )
          `);
          queryValues.push(filters.dateRange.from, filters.dateRange.to);
          countValues.push(filters.dateRange.from, filters.dateRange.to);
        ...
        if (filters.dateRange.from === filters.dateRange.to && isToday && fromTimeInMinutes > currentTimeInMinutes) {
          // Looking at today and timeFrom is in the future, include previous day
          const prevDay = new Date(filters.dateRange.from);
          prevDay.setDate(prevDay.getDate() - 1);
          const prevDayStr = prevDay.toISOString().split('T')[0];
          ...
          conditions.push(`
            (
              (pr.timestamp::date = $${paramIndex - 2} AND
               EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) >= ${fromTimeInMinutes})
              OR
              pr.timestamp::date = $${paramIndex - 1}
            )
          `);
        } else {
          conditions.push(`
        ...
        if (filters.dateRange.from === filters.dateRange.to) {
          ...
          conditions.push(`
            (
              pr.timestamp::date < $${paramIndex} OR
              (pr.timestamp::date = $${paramIndex} AND
               EXTRACT(HOUR FROM pr.timestamp) * 60 + EXTRACT(MINUTE FROM pr.timestamp) <= ${toTimeInMinutes})
            )
          `);
          queryValues.push(filters.dateRange.to);
          countValues.push(filters.dateRange.to);
          paramIndex++;
        }
      }
    }
  } else if (filters.timeFrom || filters.timeTo) {
    // No date range specified, use current date and possibly previous day
    const now = new Date();
    const currentTimeInMinutes = now.getHours() * 60 + now.getMinutes();
    const currentDate = now.toISOString().split('T')[0];
    
    const [fromHours, fromMinutes] = filters.timeFrom?.split(':').map(Number) || [0, 0];
    const fromTimeInMinutes = fromHours * 60 + fromMinutes;
    const [toHours, toMinutes] = filters.timeTo?.split(':').map(Number) || [23, 59];
    const toTimeInMinutes = toHours * 60 + toMinutes;

    if (toTimeInMinutes < fromTimeInMinutes || (filters.timeFrom && fromTimeInMinutes > currentTimeInMinutes)) {
      ...
      conditions.push(`pr.timestamp::date BETWEEN $${paramIndex} AND $${paramIndex + 1}`);
      queryValues.push(prevDayStr, currentDate);
      countValues.push(prevDayStr, currentDate);
      paramIndex += 2;
      ...
      conditions.push(`pr.timestamp::date = $${paramIndex}`);
      queryValues.push(currentDate);
      countValues.push(currentDate);
      paramIndex++;
  ...
  const dataQuery = `
    SELECT 
      ...
      /* Add this to get the parent plate number for misreads */
      (SELECT parent_plate_number 
       FROM known_plates 
       WHERE plate_number = pr.plate_number) as parent_plate_number,
      COALESCE(
        kp.name,
        (SELECT kp2.name 
         ...
         WHERE kp2.plate_number = 
           (SELECT parent_plate_number 
            FROM known_plates 
            WHERE plate_number = pr.plate_number))
      ) as parent_known_name,
      kp.name as known_name,
      /* Tags handling */
      COALESCE(
        (
          SELECT array_agg(DISTINCT jsonb_build_object('name', t.name, 'color', t.color))
          FROM plate_tags pt 
          JOIN tags t ON pt.tag_id = t.id 
          WHERE pt.plate_number = pr.plate_number
        ),
        ARRAY[]::jsonb[]
      ) as tags,
      /* Parent tags for misreads */
      COALESCE(
        (
          SELECT array_agg(DISTINCT jsonb_build_object('name', t.name, 'color', t.color))
          FROM known_plates kp_parent
          JOIN plate_tags pt ON pt.plate_number = kp_parent.plate_number
          JOIN tags t ON pt.tag_id = t.id 
          WHERE kp_parent.plate_number = 
            (SELECT parent_plate_number 
             FROM known_plates 
             WHERE plate_number = pr.plate_number)
        ),
...
export const addKnownPlateWithMisreads = addKnownPlateWithMisreadsDB;

export async function addKnownPlateWithMisreadsDB({ plateNumber, name, notes, misreads }) {
  const client = await pool.connect();
  ...
  try {
    ...
    const plateResult = await client.query(
      `INSERT INTO known_plates (plate_number, name, notes)
       ...
       ON CONFLICT (plate_number) 
       DO UPDATE SET 
         name = COALESCE($2, known_plates.name),
         notes = COALESCE($3, known_plates.notes)
       RETURNING *`,
      [plateNumber, name, notes]
    );
...
function relativeTime(date) {
  ...
  try {
    ...
    if (diffInTime < 1000 * 60 * 60 * 24 * 2) return 'Yesterday';
    const diffInDays = Math.floor(diffInTime / (1000 * 60 * 60 * 24));
    if (diffInDays >= 15) return '15+ days ago';
          ...
          SELECT json_agg(
            json_build_object(
              ...
              'last_seen_at', CASE 
                ...
                ELSE NULL
              END
            ) ORDER BY COALESCE(rc2.last_seen_at, '1970-01-01'::timestamp) DESC
          )
          FROM known_plates kp2
